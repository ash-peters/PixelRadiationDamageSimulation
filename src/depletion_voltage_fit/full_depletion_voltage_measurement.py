...
basename = os.path.basename(args.input_file_path)
match = re.search(r'Ntuple_(\d+)_\d+\.root', basename)

if not match:
    raise ValueError(f"Could not extract run number from file name: {basename}")
...
output_filename = os.path.join(output_dir, f"depletion_results_run{run_number}.csv")
...
for directory in directories:
    ...
    for key in directory.GetListOfKeys():
        ...
        with open(output_filename, "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([run_number, section_name, hist.GetName(), intersection_y])
